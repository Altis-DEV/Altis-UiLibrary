...
--// Interactive GUI registry
ImGui.__Interactions = setmetatable({}, {__mode = "k"})

function ImGui:RegisterInteraction(Window, GuiObject)
	if not Window or not GuiObject then return GuiObject end
	local List = self.__Interactions[Window]
	if not List then
		List = {}
		self.__Interactions[Window] = List
	end
	if not table.find(List, GuiObject) then
		table.insert(List, GuiObject)
	end
	return GuiObject
end

function ImGui:UnregisterInteraction(Window, GuiObject)
	local List = self.__Interactions[Window]
	if not List then return end
	for Index = #List, 1, -1 do
		if List[Index] == GuiObject then table.remove(List, Index) end
	end
end

function ImGui:IsInteractionAt(Window, Position)
	local List = self.__Interactions[Window]
	if not List then return false end
	for Index = #List, 1, -1 do
		local GuiObject = List[Index]
...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:NewInstance(Instance: Frame, Class, Parent)
		...
		if Window and (Instance:IsA("GuiButton") or Instance:IsA("TextBox") or Instance:IsA("ScrollingFrame")) then
			ImGui:RegisterInteraction(Window, Instance)
		end
	...
	if IsProgress and UIPadding then
	...
	function Config:SetValue(NewValue: number, SliderMode: boolean)
		local MinValue = Config.MinValue or 0
		local MaxValue = Config.MaxValue or 100
		local Difference = MaxValue - MinValue
		if Difference == 0 then Difference = 1 end
		local Percentage
		if SliderMode then
			Percentage = math.clamp(tonumber(NewValue) or 0, 0, 1)
			NewValue = MinValue + Difference * Percentage
		else
			NewValue = tonumber(NewValue) or MinValue
			Percentage = math.clamp((NewValue - MinValue) / Difference, 0, 1)
		end

		local Props = IsProgress and {Size = UDim2.fromScale(Percentage, 1)} or {Position = UDim2.fromScale(Percentage, 0.5)}
		ImGui:Tween(Grab, Props)
		Config.Value = NewValue
		ValueText.Text = ValueFormat:format(NewValue, MaxValue)
		Callback(NewValue)
		return Config
	end
	...
	local function UpdateFromInput(Input)
		...
		local Percentage = (Input.Position.X - Slider.AbsolutePosition.X) / Width
		Config:SetValue(math.clamp(Percentage, 0, 1), true)
	end

	Slider.InputBegan:Connect(function(Input)
		local T = Input.UserInputType
		if T ~= Enum.UserInputType.MouseButton1 and T ~= Enum.UserInputType.Touch then return end
		if Config.ReadOnly or Dragging then return end
		Dragging = true
		ActiveInput = Input
	...
	UserInputService.InputChanged:Connect(function(Input)
		...
		if Input.UserInputType == Enum.UserInputType.MouseMovement then
			if ActiveInput and ActiveInput.UserInputType == Enum.UserInputType.MouseButton1 then UpdateFromInput(Input) end
...
function ImGui:ApplyDraggable(Frame: Frame, Header: Frame)
	...
	local function End(Input)
		if not Dragging then return end
		if Input and Input ~= ActiveInput then return end
		Dragging = false
		ActiveInput = nil
		DragStart = nil
		StartPosition = nil
	...
	local function Update(Position)
		...
		local Current = Vector2.new(Position.X, Position.Y)
	...
	UserInputService.InputBegan:Connect(function(Input)
		...
		if not IsInside(Frame, Position) then return end
		if ImGui:IsInteractionAt(Frame, Position) then return end

		if T == Enum.UserInputType.MouseButton1 then
			if IsInside(Header, Position) then Begin(Input) end
		else
			Begin(Input)
		end
	end)
	...
	UserInputService.InputChanged:Connect(function(Input)
		if not Dragging then return end
		if Input.UserInputType == Enum.UserInputType.MouseMovement then
			if ActiveInput and ActiveInput.UserInputType == Enum.UserInputType.MouseButton1 then Update(Input.Position) end
		elseif Input.UserInputType == Enum.UserInputType.Touch and Input == ActiveInput then
			Update(Input.Position)
		end
	end)

	UserInputService.InputEnded:Connect(function(Input) End(Input) end)
end
...
function ImGui:ApplyResizable(MinSize, Frame: Frame, Dragger: TextButton, Config)
	...
	ImGui:RegisterInteraction(Frame, Dragger)
	...
	UserInputService.InputChanged:Connect(function(Input)
		if not DragStart or not OriginalSize then return end
		if Input.UserInputType == Enum.UserInputType.MouseMovement then
		...
		local Size = UDim2.fromOffset(math.max(MinSize.X, NewSize.X), math.max(MinSize.Y, NewSize.Y))
		Frame.Size = Size
		if Config then Config.Size = Size end
	end)
...
function ImGui:CreateWindow(WindowConfig)
	...
	if typeof(WindowConfig.Size) == "Vector2" then
		WindowConfig.Size = UDim2.fromOffset(WindowConfig.Size.X, WindowConfig.Size.Y)
	end
	...
	pcall(function() Resize.Size = UDim2.fromOffset(25, 25) end)
	pcall(function() Resize.TextSize = 24 end)
	...
	-- Keep the original toolbar sizing; do not use AbsoluteSize for layout.
	local OriginalToolBarSize = ToolBar.Size
	local OriginalToolBarAutomaticSize = ToolBar.AutomaticSize

	local TabBar: ScrollingFrame = Instance.new("ScrollingFrame")
	TabBar.Name = "TabBar"
	...
	-- Keep the template in the original toolbar so its layout/height remains
	-- exactly as authored by the prefab. Actual buttons go into TabBar.
	ToolBar.TabButton.Visible = false

	-- Restore the original sizing rules after adding the scroller.
	ToolBar.Size = OriginalToolBarSize
	ToolBar.AutomaticSize = OriginalToolBarAutomaticSize

	ImGui:RegisterInteraction(Window, TabBar)

	WindowConfig.__TabBar = TabBar
	WindowConfig.__TabLayout = TabLayout

	-- Restore the prefab's own toolbar size. No AbsoluteSize-derived
	-- height is used here.
	ToolBar.Size = OriginalToolBarSize
	ToolBar.AutomaticSize = OriginalToolBarAutomaticSize
	...
	--// Close Window 
	local CloseButton: GuiObject = TitleBar.Close
	CloseButton.Visible = WindowConfig.NoClose ~= true

	local CloseAsset = "rbxassetid://127173792845658"

	if CloseButton:IsA("ImageButton") then
		CloseButton.Image = CloseAsset
	elseif CloseButton:IsA("TextButton") then
		CloseButton.Text = ""
		for _, Descendant in next, CloseButton:GetDescendants() do
		...
		Icon.Name = "AltisCloseIcon"
		Icon.BackgroundTransparency = 1
		Icon.AnchorPoint = Vector2.new(0.5, 0.5)
		Icon.Position = UDim2.fromScale(0.5, 0.5)
		Icon.Size = UDim2.fromScale(0.78, 0.78)
		Icon.Image = CloseAsset
		Icon.ScaleType = Enum.ScaleType.Fit
		Icon.ZIndex = CloseButton.ZIndex + 1
		Icon.Parent = CloseButton
	end

	pcall(function()
		local S = CloseButton.Size
		CloseButton.Size = UDim2.new(
			S.X.Scale, S.X.Offset + 4,
			S.Y.Scale, S.Y.Offset + 4
		)
	end)

	ImGui:RegisterInteraction(Window, CloseButton)
	...
	function WindowConfig:CreateTab(Config)
		...
		ImGui:RegisterInteraction(Window, TabButton)
