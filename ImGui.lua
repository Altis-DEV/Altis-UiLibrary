...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:Slider(Config)
		...
		local function GetPercentage(PositionX)
		...
		local function ApplyPointerPosition(PositionX)
			...
			local Percentage = GetPercentage(PositionX)
		...
		--// End only the active interaction.
...
function ImGui:ApplyDraggable(Frame: Frame, Header: GuiObject)
	...
	local function ShouldIgnore(Position)
		...
		for _, Object in ipairs(Objects) do
			...
			if Object:IsDescendantOf(Frame) then
				-- Any active input object inside the header gets
				-- its own interaction instead of moving the window.
	...
	-- PC and mobile both use the same global hit test.
	UserInputService.InputBegan:Connect(function(Input)
		...
		-- Drag is restricted to the requested Header.
		if not IsInside(Header, Position) then
			return
		end

		if ShouldIgnore(Position) then
			return
		end
	...
	UserInputService.InputChanged:Connect(function(Input)
		...
		local Position = GetPosition(Input)
		local Delta = Position - StartInputPosition
...
function ImGui:ApplyResizable(MinSize, Frame: Frame, Dragger: TextButton, Config)
	...
	local function Update(Input)
		if not ResizeStart or not OriginalSize then
			return
		end
...
function ImGui:CreateWindow(WindowConfig)
	...
	--// Preserve the prefab toolbar geometry. Never derive toolbar
	--// height from child content; the toolbar belongs to the window.
	local ToolBarSize = ToolBar.Size

	--// Horizontal tab scroller.
	local TabBar = ToolBar:FindFirstChild("TabBar")
	if not TabBar then
		...
		local ExistingLayout = ToolBar:FindFirstChildOfClass("UIListLayout")
		if ExistingLayout then
			ExistingLayout.Parent = TabBar
		end
		...
		WindowConfig.TabBarLayout = Layout
		...
		local function UpdateTabBar()
			...
			local ViewportWidth = TabBar.AbsoluteSize.X
			...
			if ContentWidth > ViewportWidth + 1 then
				TabBar.ScrollBarThickness = WindowConfig.TabBarScrollBarThickness or 4
			else
				TabBar.ScrollBarThickness = 0
				TabBar.CanvasPosition = Vector2.zero
			end
		...
		Layout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(UpdateTabBar)
		TabBar:GetPropertyChangedSignal("AbsoluteSize"):Connect(UpdateTabBar)
		WindowConfig.UpdateTabBar = UpdateTabBar
	end
	...
	function WindowConfig:CreateTab(Config)
		...
		TabButton.Parent = WindowConfig.TabBar or ToolBar
	...
	function WindowConfig:ShowTab(TabClass: SharedTable)
		...
		--// Ensure active page fills the viewport when content is short.
		local Layout = TargetPage:FindFirstChildOfClass("UIListLayout")
		local Padding = TargetPage:FindFirstChildOfClass("UIPadding")
		local ContentHeight = Body.AbsoluteWindowSize.Y
	...
	--// Re-apply requested window size after style assignment.
	if typeof(WindowConfig.Size) == "UDim2" then
		Window.Size = WindowConfig.Size
	end

	--// Toolbar size always follows the prefab, never child content.
