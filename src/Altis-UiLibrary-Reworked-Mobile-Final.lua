...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:NewInstance(Instance: Frame, Class, Parent)
		...
		--// Register interactive controls with their Window so the
		--// mobile Window drag does not steal their touch input.
		if WindowConfig and WindowConfig.__RegisterInteraction then
			if Instance:IsA("GuiButton")
				or Instance:IsA("TextBox")
				or Instance:IsA("ScrollingFrame") then
				WindowConfig.__RegisterInteraction(Instance)
			end
		end
	...
	function ContainerClass:Checkbox(Config)
		...
		if WindowConfig and WindowConfig.__RegisterInteraction then
			WindowConfig.__RegisterInteraction(CheckBox)
		end
	...
	function ContainerClass:CollapsingHeader(Config)
		...
		if WindowConfig and WindowConfig.__RegisterInteraction then
			WindowConfig.__RegisterInteraction(Header)
		end
	...
	function ContainerClass:Slider(Config)
		...
		if WindowConfig and WindowConfig.__RegisterInteraction then
			WindowConfig.__RegisterInteraction(Slider)
		end
		...
		local function UpdateFromInput(Input)
			if Config.ReadOnly or not Dragging then return end
			local Width = Slider.AbsoluteSize.X
			if Width <= 0 then return end
			local Percentage = math.clamp(
				(Input.Position.X - Slider.AbsolutePosition.X) / Width,
				0,
				1
			)
		...
		Slider.InputBegan:Connect(function(Input)
			...
			if Type ~= Enum.UserInputType.MouseButton1
				and Type ~= Enum.UserInputType.Touch then return end
			Dragging = true
			ActiveInput = Input
	...
	function ContainerClass:Keybind(Config)
		...
		if WindowConfig and WindowConfig.__RegisterInteraction then
			WindowConfig.__RegisterInteraction(Keybind)
		end
	...
	function ContainerClass:Combo(Config)
		...
		if WindowConfig and WindowConfig.__RegisterInteraction then
			WindowConfig.__RegisterInteraction(Combo)
		end
...
function ImGui:ApplyDraggable(Frame: Frame, Header: Frame)
	...
	local function RegisterInteraction(GuiObject)
		if not GuiObject then return end
		if table.find(Interactions, GuiObject) then return end
		table.insert(Interactions, GuiObject)
	end

	Frame:SetAttribute("ImGuiDragRoot", true)
	...
	local function IsInteractionPoint(Position)
		for Index = #Interactions, 1, -1 do
			local GuiObject = Interactions[Index]
			...
			if IsInside(GuiObject, Position) then
				return true
			end
		end
	...
	local function BeginDrag(Input)
		...
		if Type == Enum.UserInputType.MouseButton1 then
			if not IsInside(Header, Position) then
				return
			end
		end

		if IsInteractionPoint(Position) then
			return
		end

		Dragging = true
		ActiveInput = Input
		KeyBeganPos = Input.Position
...
function ImGui:ApplyResizable(MinSize, Frame: Frame, Dragger: TextButton, Config)
	...
	Dragger.Size = UDim2.fromOffset(25, 25)
	Dragger.TextSize = 24
	...
	local function UpdateResize(Position)
		if not Resizing or not DragStart or not OriginalSize then return end

		local Current = Vector2.new(Position.X, Position.Y)
		local Delta = Current - DragStart

		local NewSize = UDim2.fromOffset(
			math.max(MinSize.X, OriginalSize.X + Delta.X),
			math.max(MinSize.Y, OriginalSize.Y + Delta.Y)
		)
	...
	Dragger.InputBegan:Connect(BeginResize)
...
function ImGui:CreateWindow(WindowConfig)
	...
	-- Normalize size before layout calculations.
	...
	pcall(function() CloseButton.Image = "rbxassetid://127173792845658" end)
	pcall(function() CloseButton.Size = UDim2.fromOffset(28, 28) end)
	pcall(function() CloseButton.Text = "" end)

	--// Toolbar
	local ToolBar = Content.ToolBar
	ToolBar.Visible = WindowConfig.TabsBar ~= false

	-- Keep every existing toolbar property/layout from the prefab.
	-- The new ScrollingFrame is only a viewport for the tab buttons.
	...
	-- The original TabButton is still the template; it is not part of
	-- the scrolling content.
	ToolBar.TabButton.Visible = false

	local RegisterInteraction

	--// Window Drag
	if not WindowConfig.NoDrag then
		RegisterInteraction = ImGui:ApplyDraggable(Window, TitleBar)
	end

	WindowConfig.__RegisterInteraction = RegisterInteraction

	-- Resize is always protected from Window drag.
	if RegisterInteraction then
		RegisterInteraction(Resize)
		RegisterInteraction(CloseButton)
		RegisterInteraction(Toggle.ToggleButton)
		RegisterInteraction(TabBar)
	end
	...
	function WindowConfig:SetOpen(Open: true, NoAnimation: false)
		...
		ImGui:Tween(Resize, {TextTransparency = Open and 0.6 or 1, Interactable = Open}, nil, NoAnimation)
		ImGui:Tween(Window, {Size = Open and self.Size or UDim2.fromOffset(WindowAbSize.X, TitleBarSize.Y)}, nil, NoAnimation)
		ImGui:Tween(Body, {Visible = Open}, nil, NoAnimation)
		return self
	end
	...
	function WindowConfig:CreateTab(Config)
		...
		if RegisterInteraction then
			RegisterInteraction(TabButton)
		end

		local AutoSizeAxis = WindowConfig.AutoSize or "Y"
		local ContentFrame: Frame = Body.Template:Clone()
		ContentFrame.AutomaticSize = Enum.AutomaticSize[AutoSizeAxis]
		...
		elseif AutoSizeAxis == "X" then
			ContentFrame.Size = UDim2.fromScale(0, 1)
		end
		...
		Config = ImGui:ContainerClass(ContentFrame, Config, Window)
	...
	function WindowConfig:SetSize(Size)
		...
		local NewSize = UDim2.new(
			...
			Size.Y.Scale, Size.Y.Offset + HeaderSizeY
		)
		self.Size = NewSize
		Window.Size = NewSize
		return self
	...
	function WindowConfig:ShowTab(TabClass: SharedTable)
		...
		for _, Page in next, Body:GetChildren() do
			if Page:IsA("GuiObject") and Page ~= Body.Template then
