...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:NewInstance(Instance: Frame, Class, Parent)
		...
		-- Interactive controls consume the touch/click instead of
		-- allowing the parent Window to start dragging.
		if Window
			and (
...
function ImGui:ApplyDraggable(Frame: Frame, Header: GuiObject?)
	...
	ImGui.__DragInteractions =
		ImGui.__DragInteractions or {}

	ImGui.__DragInteractions[Frame] =
		ImGui.__DragInteractions[Frame] or {}
	...
	local function IsProtectedInteraction(Position: Vector2)
		...
		for Index = #Interactions, 1, -1 do
			...
			if IsInside(Object, Position) then
				return true
			end
		end
	...
	function ImGui:RegisterDragInteraction(
		Window: GuiObject,
		GuiObject: GuiObject
	)
		if not Window or not GuiObject then
			return GuiObject
		end

		self.__DragInteractions =
			self.__DragInteractions or {}

		self.__DragInteractions[Window] =
			self.__DragInteractions[Window] or {}
	...
	-- Global capture is intentional:
	-- a touch that begins on a child GuiObject may not bubble to
	-- the Window or TitleBar. This is the same principle used by
	-- the working modular Window implementation.
	UserInputService.InputBegan:Connect(function(Input)
		...
		-- Controls that registered themselves get the input.
		if IsProtectedInteraction(
			Position
		) then
			return
		end
		...
		-- Touch:
		-- Anywhere inside the Window that is not an interactive
		-- registered control can move the Window.
		Begin(Input)
	end)
...
\t-- Do not change the prefab toolbar's own size.
\t-- Its original layout determines the toolbar height.
\t-- The scrolling container simply fills that existing area.
...
\t-- Original TabButton is only a prefab.
...
\t-- The prefab's close control can be a TextButton with an
\t-- image child, or an ImageButton. Handle both safely.
\tpcall(function()
...
\tif CloseButton:IsA("ImageButton")
\t\tor CloseButton:IsA("ImageLabel") then

\t\tIconObject = CloseButton
\telse
\t\tIconObject =
\t\t\tCloseButton:FindFirstChildWhichIsA(
\t\t\t\t"ImageLabel",
\t\t\t\ttrue
\t\t\t)
\t\t\tor CloseButton:FindFirstChildWhichIsA(
\t\t\t\t"ImageButton",
\t\t\t\ttrue
\t\t\t)
\tend
...
\telse
\t\t-- Fallback for a text-only prefab: create a dedicated icon.
\t\tlocal Icon =
\t\t\tInstance.new("ImageLabel")

\t\tIcon.Name = "CloseIcon"
\t\tIcon.BackgroundTransparency = 1
\t\tIcon.Image = CloseIconId
...
\t-- Slightly enlarge the close button itself.
\tpcall(function()
\t\tlocal CurrentSize =
\t\t\tCloseButton.Size
...
\tend)
	...
	-- Restore the tab bar geometry after style application.
