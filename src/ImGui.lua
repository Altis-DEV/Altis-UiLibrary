...
--//   Base        -> optional parent theme
--//   Colors      -> element/descendant color overrides
--//   Properties  -> element/descendant property overrides
--//   Global      -> properties applied to every GuiObject
--//   Animations  -> animation preset overrides
--//
--// Colors/Properties can target any object name in the prefab or
--// any object created later by ContainerClass.
--// ============================================================
...
local function DeepMerge(Base, Override)
	...
	for Key, Value in next, Override or {} do
		if typeof(Value) == "table"
			and typeof(Result[Key]) == "table" then
...
local function CleanObjectName(Name)
	return tostring(Name):gsub("_+$", "")
end

local function ApplyThemeObject(GuiObject, Theme)
	if not GuiObject
		or not GuiObject:IsA("GuiObject") then
	...
	local Properties =
		Theme.Properties
		and Theme.Properties[ElementType]
...
local function ApplyThemeToWindow(Window, Config, Theme)
	if not Window or not Theme then
		return
	end
	...
	Config.__ThemeColors =
		DeepCopy(
			Theme.Colors or {}
		)

	Config.__ThemeProperties =
		DeepCopy(
			Theme.Properties or {}
		)

	Config.__ThemeAnimations =
		DeepCopy(
			Theme.Animations or
			ImGui.Animations
		)

	--// Keep legacy Colors API.
	Config.Colors =
		DeepMerge(
			Theme.Colors or {},
	...
	--// Window-specific custom colors always win.
	if Config.__CustomColors then
		ImGui:ApplyColors(
			Config.__CustomColors,
			Window,
			"Window"
		)
	end

	--// Window-specific custom properties always win.
	if Config.__CustomProperties then
		for Property, Value in next,
			Config.__CustomProperties do
...
function ImGui:AddTheme(Theme)
	assert(
		typeof(Theme) == "table",
		"ImGui:AddTheme expects a table"
	)

	assert(
		typeof(Theme.Name) == "string"
			and Theme.Name ~= "",
		"ImGui:AddTheme: Theme.Name is required"
	)
...
function ImGui:SetTheme(ThemeName)
	...
	for Window, Config in next,
		self.Windows do

		-- Explicit window themes are independent
		-- from the global theme.
		if Config.__ExplicitTheme ~= true then
			Config.Theme =
				Key
...
--// DEFAULT THEME
--// Empty colors preserve the original prefab appearance.
local DefaultTheme = {
	Name = "default",
...
--// LIGHT THEME
--// Covers Window, every public Element prefab and common
--// descendants. Users can add arbitrary additional object names.
local LightTheme = {
	Name = "light",
...
function ImGui:ContainerClass(Frame: Frame, Class, Window)
	...
	function ContainerClass:NewInstance(Instance: Frame, Class, Parent)
		...
		--// Mark this object as an ImGui element.
		--// Window drag uses this marker to avoid dragging the
		--// entire Window while a control is being manipulated.
		pcall(function()
			Instance:SetAttribute("ImGuiElement", true)
		end)
		...
		--// Apply the active window theme to newly-created elements.
		if WindowConfig.__ThemeData then
			ApplyThemeObject(
				Instance,
				WindowConfig.__ThemeData
			)
		end
	...
	function ContainerClass:Slider(Config)
		...
		--// Slider element
		local Slider: TextButton =
			Prefabs.Slider:Clone()

		local UIPadding =
			Slider:FindFirstChildOfClass(
				"UIPadding"
			)
...
function ImGui:ApplyDraggable(
	...
	local function IsElementAt(Position)
		local GuiObjects =
		...
		for _, GuiObject in
			...
			if not GuiObject:IsDescendantOf(Frame) then
				continue
			end

			if GuiObject == Frame
				or GuiObject == Header then
				continue
			end

			-- Any explicitly registered ImGui element blocks
			-- Window drag.
			local Current =
				GuiObject

			while Current
				and Current ~= Frame do
			...
			-- Native interactive objects also block dragging.
			if GuiObject:IsA("GuiButton")
				or GuiObject:IsA("TextBox")
				or GuiObject:IsA("ScrollingFrame") then
	...
	local function BeginDrag(Input)
		...
		if Type
			...
			-- Mobile can start a Window drag anywhere
			-- except directly on an ImGui control.
			if not IsInside(
				Frame,
				Position
			) then
			...
			if IsElementAt(Position) then
				return
			end
		else
			-- PC uses the TitleBar.
			if not IsInside(
				Header,
				Position
			) then
			...
			-- Buttons/TextBoxes/marked controls inside the TitleBar
			-- must keep their own input.
			if IsElementAt(Position) then
				return
			end
		end
...
function ImGui:ApplyResizable(
	...
	local function UpdateResize(Position)
		if not Resizing
			or not DragStart
...
function ImGui:CreateWindow(WindowConfig)
	...
	--// Remember whether the caller explicitly selected a theme.
	local ExplicitTheme =
		WindowConfig.Theme ~= nil
	...
	ToolBar.Visible =
		WindowConfig.TabsBar
		~= false

	--// Preserve the original toolbar dimensions.
	local ToolBarOriginalSize =
		ToolBar.Size

	ToolBar.AutomaticSize =
		Enum.AutomaticSize.None

	--// Remove the original direct-child layout if present.
	local ToolBarLayout =
		ToolBar:FindFirstChildOfClass(
			"UIListLayout"
		)
	...
	--// Horizontal toolbar ScrollingFrame.
	local TabBar =
		Instance.new(
			"ScrollingFrame"
		)
	...
	-- Requested automatic horizontal expansion.
	...
	TabBar.ScrollBarThickness =
		WindowConfig.TabBarScrollBarThickness
		or 4
	...
	--// Tab layout inside the scrolling toolbar.
	local TabLayout =
		Instance.new(
			"UIListLayout"
		)
	...
	--// Copy toolbar padding to the scrolling container.
	...
	if ToolBarPadding then
		...
		Padding.PaddingBottom =
			ToolBarPadding.PaddingBottom

		Padding.Parent =
			TabBar
	end

	--// Original button is a template.
	ToolBar.TabButton.Visible =
		false

	--// Keep original toolbar height.
	ToolBar.Size =
		ToolBarOriginalSize
	...
	function WindowConfig:CreateTab(Config)
		...
		function Config:GetContentSize()
			return ContentFrame.AbsoluteSize
		end

		--// Preserve the original element system.
		Config =
		...
		if WindowConfig.AutoSize then
			ContentFrame:GetPropertyChangedSignal(
				"AbsoluteSize"
			):Connect(
				function()
					local Size =
	...
	function WindowConfig:ShowTab(
		...
		-- Reset the Body scroll when switching tabs.
		Body.CanvasPosition =
			Vector2.zero
	...
	WindowConfig.__ExplicitTheme =
		ExplicitTheme

	-- Keep caller-supplied legacy colors/properties.
	WindowConfig.__CustomColors =
		DeepCopy(
			WindowConfig.Colors or {}
		)

	WindowConfig.__CustomProperties =
		DeepCopy(
			WindowConfig.Properties or {}
		)
	...
	if Theme then
		...
		WindowConfig.__ThemeAnimations =
			Theme.Animations
	end
	...
	-- Register before applying dynamic theme.
	...
	-- Re-apply requested size after style writes.
	if typeof(WindowConfig.Size)
		== "UDim2" then
	...
	-- Re-apply toolbar size after theme/style processing.
	ToolBar.Size =
		ToolBarOriginalSize
